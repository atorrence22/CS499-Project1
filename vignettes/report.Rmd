...
for(data.name in names(data.list))
{
  ...
  nearestNeighbors(one.data.set$feature.mat, one.data.set$label.vec)
  
  for(test.fold in 1:n.folds){
    is.train <- fold.vec != test.fold
    fit <- nearestNeighbors(one.data.set$feature.mat[is.train,], one.data.set$label.vec[is.train])
